...
- **Blue** — GCP managed services (Cloud DNS, Secret Manager, GCS,
  Workload Identity Federation, Loadbalancer). Operated by Google;
  configured via Terraform on our side.
...
**Day 1** is the one-time bootstrap, executed by Terraform in the
[`platform-iac`](https://github.com/INENI-PT-GROUP-B/platform-iac)
repository. Terraform creates the GCP project artefacts (VPC, GKE
cluster, Cloud DNS zone bindings, Workload Identity Federation, IAM
service accounts), installs Argo CD via its Helm chart, and creates
one root Argo CD `Application` pointing at `platform-gitops`.
...
A second `terraform apply` is only needed for changes at the cluster
topology level (new node pool, VPC change, IAM role addition).
...
- `app-chart` (public, OCI): the Helm chart that describes the app
  deployment. Pulled by Crossplane.
...
cannot be generated inside the cluster: image pull credentials for
GHCR (manually placed once) and tenant application login passwords
(generated by Crossplane during tenant onboarding). Database
credentials are **not** stored in GSM — see CloudNativePG below.
...
**Workload Identity Federation (OIDC)** — the trust chain between
GitHub Actions and GCP IAM Service Accounts. Allows CI jobs to
authenticate against GCP without long-lived JSON keys.
...
provisioning via a Composition. Authenticates against GCP via Workload
Identity.
...
GSM (also via Workload Identity). For each tenant, Crossplane creates
`ExternalSecret` objects for the GHCR pull secret and the application
login password; ESO materialises them as Kubernetes `Secret` objects
...
**Workload Identity bindings** — a set of RoleBindings linking
Kubernetes ServiceAccounts to GCP IAM Service Accounts. This allows
pods (e.g. ESO, ExternalDNS, cert-manager) to call GCP APIs without
JSON keys living in the cluster.
...
cluster in the same namespace). Prevents workloads from communicating
directly between tenants.
...
**ExternalSecrets** — two secret types are synced from GSM per tenant:
- **GHCR pull secret** — image pull credential for the private
  frontend image. Placed manually in GSM once (cannot be auto-generated).
- **App login credentials** — the random plaintext password that
  guards the tenant's application. Generated by Crossplane during
  onboarding and stored in GSM. Synced into the tenant namespace as
  the environment variable `APP_PASSWORD`. The backend hashes the
  password on startup (bcrypt, in memory) and verifies login requests
  against the in-memory hash. The plaintext never leaves GSM and the
  backend's process memory.

Database credentials are **not** here; they come directly from the
secret that CloudNativePG creates next to the database cluster.
...
backend URL is injected at runtime through `/config.js`.

**Backend** — Deployment exposing the REST API on port 3000. Health
endpoint at `GET /healthz`. On startup, connects to the database
using `DATABASE_URL` from the CloudNativePG-managed secret, runs
idempotent schema migrations, then hashes the `APP_PASSWORD` value
into an in-memory hash before accepting requests.
...
**GitHub Actions** — runs three categories of pipeline across the
six repositories:
...
- **Terraform apply** in `platform-iac`: on merge to `main`, runs
  `terraform plan` and (with manual approval) `terraform apply`
  against the GCP project.

Authentication to GCP from GitHub Actions uses **Workload Identity
Federation** (OIDC). There are no long-lived service account JSON
...
3. Traefik terminates TLS with the wildcard certificate
   ...
   ingress to use.
4. Request to `/` → Traefik routes to the frontend Service in the
   namespace `tenant-a`. nginx serves the SPA assets.
5. Browser executes the SPA, which calls the backend API at
   `/api/...`. The request again goes through DNS → LB → Traefik.
6. Traefik routes `/api/*` to the backend Service in the same
   namespace.
7. Backend validates the auth cookie (or the login request itself
   against the in-memory password hash). On a valid session, it
   performs CRUD operations on the application's database tables.
8. Backend opens a database connection to the CloudNativePG cluster
   using `DATABASE_URL` from the CloudNativePG-managed secret.
9. PostgreSQL reads/writes its data files on the PVC `db-data`.
10. Response travels back to the browser along the reverse path.
...
1. A team member opens a pull request against `platform-gitops` that
   ...
   content is the tenant name plus optional configuration (e.g.
   `imageTagOverride` for a staging tenant).
...
4. Crossplane sees the new claim and reconciles the corresponding
   ...
   - the CloudNativePG `Cluster` (the CloudNativePG operator
     ...
     credentials secret)
   - a random application login password, stored in GSM, plus the
     `ExternalSecret` that makes it available in the namespace
   - the `ExternalSecret` for the GHCR pull secret
   - a `Release` resource (provider-helm) that pulls the app Helm
     chart from GHCR as an OCI artefact and unfolds it (frontend
     Deployment, backend Deployment, Services, Ingress). The image
     tag comes from the central `values/app-version.yaml`, unless
     the claim sets `imageTagOverride`.
...
1. An `ExternalSecret` resource in the tenant namespace declares the
   target secret in GSM (e.g. `tenant-a/app-password` for the login
   password, or `shared/ghcr-pull-secret` for the image pull
   credential).
2. ESO authenticates against GSM via Workload Identity (no static
   key inside the cluster).
3. ESO retrieves the current secret value and writes it to a
   Kubernetes Secret in the tenant namespace.
4. Pods in the tenant namespace mount the Secret as environment
   variables (for `APP_PASSWORD`) or reference it as `imagePullSecret`
...
4. Authentication against GCS via Workload Identity, bound to the
   tenant-specific ServiceAccount.

### Application updates and staging

The application version that every tenant runs is controlled from a
single file in `platform-gitops`: `values/app-version.yaml`. It holds
the current image tag and is referenced by the Crossplane Composition
when it renders each tenant's Helm release.

**Rolling out a new version to all tenants** is a single pull request:
update the image tag in `values/app-version.yaml`, merge. Argo CD
...
**Testing a new version on a staging tenant first** uses the optional
`imageTagOverride` field in a tenant claim. The dedicated `staging`
tenant (and only that one) sets this field. When Crossplane renders
the staging tenant's release, the override beats the central value.
A typical update sequence is:

1. PR 1: set `imageTagOverride: v2.0.0` in `tenants/staging.yaml`.
   Merge, observe `staging.fhuebung.lol` on the new version.
2. Verify functionality on staging.
3. PR 2: change the central tag in `values/app-version.yaml` to
   `v2.0.0`. Merge. All other tenants roll forward.
4. PR 3: optionally remove the override from `tenants/staging.yaml`
   so staging follows the central value again.
...
- **Crossplane orchestrates the entire tenant deployment**, including
  ...
  takes over namespace, DB, secrets, network policies, quotas, and
  the Helm release for the app. This split satisfies the requirement
  "Crossplane to handle all other deployment steps" from the
  assignment and at the same time captures the Helm chart bonus.
...
- **Path-based routing** (`/api` for the backend) instead of
  ...
  level of subdomain.
- **Single-password authentication per tenant** instead of a full
  identity provider. The password is generated by Crossplane,
  stored in GSM, and hashed in the backend on startup.
...
The GKE cluster is a zonal Standard cluster with a single default
node pool of 3–6 `e2-standard-4` machines, governed by the GKE
Cluster Autoscaler. The lower bound covers the platform baseline
(Argo CD, Crossplane, ESO, ExternalDNS, cert-manager, Traefik,
CloudNativePG operator, kube-prometheus-stack) plus a handful of
small tenants; the upper bound provides headroom for tenant growth
during the project.
