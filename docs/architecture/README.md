...
- `shared-ghcr-pull-secret` — image pull credential for the private
  frontend image. Seeded by `platform-iac/bootstrap/bootstrap.sh`
  Phase 1a from an operator-supplied `GHCR_TOKEN`.
...
- **GHCR pull secret** — image pull credential for the private
  frontend image. Source: `shared-ghcr-pull-secret` in GSM, seeded
  by the bootstrap script from `GHCR_TOKEN` (the one credential the
  platform cannot machine-generate).
